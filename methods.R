...
parameter.optimise <- function(X, n.0, ground, eta, index = NULL){
   ...
   if(m == max(ari.clus))
      return(list('lambda' = lambda[r.1], 'Number of Buckets' = val[[r.1]][[3]], 
                  'Estimated number of clusters' = val[[r.1]][[2]], 
                  'NMI' = nmi.clus[r.1], 'ARI' = ari.clus[r.1],
                  'Clster assignment vector' = val[[r.1]][[6]], 
                  'Index' = val[[r.1]][[8]]))
   else if(m == max(ari.clus.second))
      return(list('lambda' = lambda.second[r.2], 'Number of Buckets' = val.second[[r.2]][[3]], 
                  'Estimated number of clusters' = val.second[[r.2]][[2]], 
                  'NMI' = nmi.clus.second[r.2], 'ARI' = ari.clus.second[r.2], 
                  'Clster assignment vector' = val.second[[r.2]][[6]], 
                  'Index' = val.second[[r.2]][[8]]))
   else return(list('lambda' = lambda.third[r.3], 'Number of Buckets' = val.third[[r.3]][[3]], 
                    'Estimated number of clusters' = val.third[[r.3]][[2]], 
                    'NMI' = nmi.clus.third[r.3], 'ARI' = ari.clus.third[r.3], 
                    'Clster assignment vector' = val.third[[r.3]][[6]], 
                    'Index' = val.third[[r.3]][[8]]))
}
